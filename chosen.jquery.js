...
(function () {
    ...
    AbstractChosen = (function () {
        ...
        AbstractChosen.prototype.results_option_build = function (options) {
            var content, data, _i, _len, _ref, data_content, shown_results;
            content = '';
            shown_results = 0;
        ...
        AbstractChosen.prototype.winnow_results = function () {
            var escapedSearchText, option, regex, results, results_group, searchText
                    , startpos, endpos, selsize, text, zregex, _i, _len, _ref, search_split, search_split_limpo, found;
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                ...
                if (this.include_option_in_results(option)) {
                    ...
                    if (!(option.group && !this.group_search)) {
                        ...
                        if (option.search_match) {
                            if (searchText.length) { // tamanho do que foi digitado
                                search_split_limpo = searchText.split(' ');
                                text = option.search_text;
                                option.search_text = '';
                                for (_s = 0, _lens = search_split.length; _s < _lens; _s++) {
                                    startpos = text.search(ar_regex[_s]); // início da seleção
                                    endpos = startpos + search_split_limpo[_s].length; // fim da seleção
                                    selsize = search_split_limpo[_s].length; // tamanho da string a selecionar
                                    option.search_text += text.substr(0, startpos);
                                    option.search_text += '<em>' + text.substr(startpos, selsize) + '</em>';
                                    text = text.substr(endpos);
                                }
                                option.search_text += text; // o que sobrou do texto sem casar
                            }
        ...
        AbstractChosen.prototype.keyup_checker = function (evt) {
            ...
            switch (stroke) {
                ...
                case 18:
                    break;
                case 46: // TECLA DELETE (DELETE KEY)
                    this.results_reset(evt); // limpa o chosen
                    break;
                default:
                    return this.results_search();
            }
    ...
    Chosen = (function (_super) {
        ...
        Chosen.prototype.results_reset = function (evt) {
            ...
            this.input_focus(evt); // se clicar em desselecionar, o campo recebe o foco
